...
- IPv4: largest, **/16**  (Host bit mask `11111111 11111111 hhhhhhhh hhhhhhhh` → 2^16 = 65536 IPs)
- IPV4: smallest, **/28**  (Host bit mask `11111111 11111111 11111111 1111hhhh` → 2^(32-28) = 2^4 = 16 IPs)
...
- `VPN: IGW (EC2-based VPN, EIP) ← vpn → IGW (EC2-based VPN, EIP)`
- `VPN: VGW (VPC) ← vpn → IGW (EC2-based VPN, VPC)`
...
VGW
- `VPN: VGW (VPC-1) ← site-to-site vpn → CGW ← site-to-site vpn →  VGW (VPC-2)`
    - supports transitive routing between VPCs
- `DX: VGW (VPC) ← private VIF → CGW`
    - supports transitive routing between VPCs
- `DX: VGW (VPC) ← vgw association → DX Gateway ← private VIF → CGW`
...
Transit Gateway
- `VPN: VPC ← vpc attachment → Transit Gateway ← site-to-site vpn, vpn attachment → CGW`
- `DX: VPC ← vpc attachment → Transit Gateway ← transit VIF → CGW`
- `DX: VPC ← vpc attachment → Transit Gateway ← tg association → DX Gateway ← transit VIF → CGW`
...
- Cannot use a DX gateway with CloudHub. Can attach a DX VIF directly to a VGW to support CloudHub.
...
- Not support: IPv6 traffic, Path MTU Discovery, AES 128-bit encryption, multicast.
...
- How to set up VPN as backup for DX?
